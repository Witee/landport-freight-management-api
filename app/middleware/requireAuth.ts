export default () => {
  return async function requireAuth(ctx, next) {
    ...
    if (path.startsWith('/api/dc/cases')) {
      ...
      if (ctx.method === 'GET') {
        ...
        if ((user && user.userId === 0) ||
            (adminUser && adminUser.userId !== undefined && adminUser.userId !== null)) {
          return await next();
        }
        // 根据解析阶段记录的错误，给出更明确提示
        const tokenError = ctx.state && ctx.state.tokenError;
        const adminTokenError = ctx.state && ctx.state.adminTokenError;
        if (tokenError === 'TokenExpiredError' || adminTokenError === 'TokenExpiredError') {
          ctx.throw(401, '登录已过期，请重新登录');
        } else if (tokenError === 'NoToken' && adminTokenError === 'NoToken') {
    ...
    // 放行其他 dc 接口（由 requireAdminAuth 处理）
