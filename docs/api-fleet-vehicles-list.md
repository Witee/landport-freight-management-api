...
- ✅ **总是返回所有车辆**：接口总是返回用户的所有车辆，即使指定日期范围内没有货运记录也会显示（统计字段为 0）
...
- **收入 (income)** = 该车辆在指定日期范围内所有货运记录的 `freight`（运费）+ `otherIncome`（其它收入）的总和
- **支出 (expense)** = 该车辆在指定日期范围内所有货运记录的 `fuelCost`（油费）+ `repairCost`（维修费）+ `accommodationCost`（住宿费）+ `mealCost`（饭费）+ `otherExpense`（其它费用）的总和
- **利润 (profit)** = `income - expense`

### 特殊情况

- 如果车辆在指定日期范围内**没有货运记录**，`income`、`expense`、`profit` 字段均为 `"0"`
